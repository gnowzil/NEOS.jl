...
const NORESULTS = :ResultsNotRetrieved
const SOLVED = :Solved
...
abstract type NEOSModel <: MPB.AbstractMathProgModel end

addemail!(m::NEOSModel, email::String) = addemail!(m.solver, email)

function MPB.optimize!(m::NEOSModel)
    ...
    #println("job $(job.number) $(job.password)")
    iterations = 0
    pollingperiod = 5.0
    offset = 0
    ...
    parseresults!(m, job)
    if m.status == NORESULTS
        @info("Try retrieving problem results from NEOS: https://neos-server.org/neos/jobs/$(10_000 * floor(Int, job.number / 10_000))/$(job.number).html")
    end
    return m.status
end
