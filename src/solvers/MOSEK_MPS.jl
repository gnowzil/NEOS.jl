...
function parse_solution!(::NEOSSolver{:MOSEK, :MPS}, m::MPSModel)
    for v in collect((m.match for m = eachmatch(r"V(\d+).+?(-?[\d\.]+e[\+\-]\d+)", m.last_results)))
...
function parse_duals!(::NEOSSolver{:MOSEK, :MPS}, m::MPSModel)
    ...
    for v in collect((m.match for m = eachmatch(r"\d+\W+C(\d+).+?\n", m.last_results)))
	s = split(v)
    ...
    for v in collect((m.match for m = eachmatch(r"\d+\W+V(\d+).+?\n", m.last_results)))
	s = split(v)
	d_lb = parse(Float64, s[7])
	d_ub = -parse(Float64, s[8])
