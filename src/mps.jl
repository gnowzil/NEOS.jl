...
MPB.LinearQuadraticModel(s::NEOSSolver{S,:MPS}) where {S} = MPSModel(s)

function neos_writexmlmodel!(m::MPSModel)
    io = IOBuffer()
    print(io, "<MPS>")
    mps_writer_sos = [(s.order, s.indices, s.weights) for s in m.sos]
    MPSWriter.write(io, m.A, m.collb, m.colub, m.f, m.rowlb, m.rowub,
                    m.sense, m.colcat, mps_writer_sos, Array{Float64}(undef,0,0))
    print(io, "</MPS>")
    # Convert the model to MPS and add
    m.xmlmodel = replace(m.solver.template, r"<MPS>.*</MPS>"is => String(take!(io)))
end

function MPB.loadproblem!(m::MPSModel, A, collb, colub, f, rowlb, rowub, sense)
...
function MPB.status(m::MPSModel)
    m.status
end

function MPB.getobjval(m::MPSModel)
    m.objVal
end

function MPB.getsolution(m::MPSModel)
    m.solution
end

function MPB.getreducedcosts(m::MPSModel)
...
function MPB.getconstrduals(m::MPSModel)
    if m.solver.provides_duals
	return 	m.duals
    else
...
function MPB.getsense(m::MPSModel)
    m.sense
end

function MPB.setsense!(m::MPSModel, sense::Symbol)
    m.sense = sense
end

function MPB.setvartype!(m::MPSModel, t::Vector{Symbol})
    m.colcat = t
end
